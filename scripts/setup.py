...
#
with open(
    os.path.join(git.getSrcRoot(), "scripts", "LONG_DESCRIPTION.txt"),
    "r",
    encoding="utf-8",
) as f:
...
#
with open(os.path.join(git.getSrcRoot(), "src", packageName, "version.py"), "w") as f:
...
setup(
    ...
    python_requires=">=3.5",
    setup_requires=["meson>=0.51.1"],
    install_requires=["meson>=0.51.1", "numpy>=1.10"],
    ...
    ext_modules=[
        Extension(
            ...
            library_dirs=[
                os.path.join(
                    git.getSrcRoot(), utils.get_build_platlib(), "benzina", "lib"
                )
            ],
