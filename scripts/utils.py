...
def get_meson_build_root(build_temp):
    ...
    mesonBuildRoot = os.path.join(
        os.path.dirname(build_temp), "meson" + mesonBuildRoot[4:]
    )
...
class build_configure(setuptools.command.build_ext.build_ext):
    ...
    user_options = [("reconfigure", "r", "Whether to forcibly reconfigure or not")]
    ...
    def run(self):
        ...
        if (
            not os.path.isfile(
                os.path.join(mesonBuildRoot, "meson-private", "coredata.dat")
            )
            or self.reconfigure
        ):
            cmd = [
            ...
            subprocess.check_call(cmd, stdin=subprocess.DEVNULL, cwd=mesonBuildRoot)
...
class build_ext(setuptools.command.build_ext.build_ext):
    def run(self):
        ...
        mesonBuildRoot = get_meson_build_root(self.build_temp)
        subprocess.check_call(["ninja"], stdin=subprocess.DEVNULL, cwd=mesonBuildRoot)
        subprocess.check_call(
            ["ninja", "install"], stdin=subprocess.DEVNULL, cwd=mesonBuildRoot
        )
    ...
    def copy_extensions_to_source(self):
        ...
        for tmpFile, stagedFile in outs.items():
            stagedFile = os.path.relpath(
                os.path.abspath(stagedFile), os.path.abspath(self.build_lib)
            )
            ...
            copy_file(srcPath, dstPath, verbose=self.verbose, dry_run=self.dry_run)
