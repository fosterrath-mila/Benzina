...
def getDWhitePoint(T=6500):
    ...
    if T >= 4000 and T <= 7000:
        p = [-4.6070e9, +2.9678e6, +0.09911e3, +0.244063]
    else:
        p = [-2.0064e9, +1.9018e6, +0.24748e3, +0.237040]
    x = np.polyval(p, 1.0 / T)
...
def getRGBtoXYZMatrix(
    r=PRIMARY_R_sRGB, g=PRIMARY_G_sRGB, b=PRIMARY_B_sRGB, w=WHITE_D65
):
    M = np.asarray(
        [
            [r[0], g[0], b[0]],
...
def getCATMatrix(wd=WHITE_D50, ws=WHITE_D65):
    ...
    Wd = np.asarray([wd[0], wd[1], 1.0 - wd[0] - wd[1]], dtype=np.float64) / wd[1]
    Ws = np.asarray([ws[0], ws[1], 1.0 - ws[0] - ws[1]], dtype=np.float64) / ws[1]
    WAd = MA.dot(Wd)
    WAs = MA.dot(Ws)
    D = np.diag(WAd / WAs)
