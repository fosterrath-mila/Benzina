...
def scaleImage(img, a):
    ...
    return cv.resize(
        img, dsize=(a.targetscale, a.targetscale), interpolation=cv.INTER_LANCZOS4
    )
...
def compressImage(img, a):
    ...
    while True:
        ...
        crfJump = (crfJump + 1) // 2
        if delta == 0:
            break  # Perfect  quality
        elif delta < 0:
            crfCurr = max(crfMin, crfOld - crfJump)  # Increase quality
        else:
            crfCurr = min(crfMax, crfOld + crfJump)  # Decrease quality
        if crfJump <= 1 and crfCurr in crfDict:
            break
    ...
    if a.verbose:
        print(
            *[
                "CRF {:2d}  PSNRHMA: {}".format(k, v["psnrhma"])
                for k, v in crfDict.items()
            ],
            sep="\n"
        )
...
def doCompressAtCRF(img, a, crf=17):
    ...
    try:
        ...
        cmdConv2H264 = [
            ...
            "stillimage",
            "-vf",
            "scale=in_range=jpeg:out_range=jpeg",
            "-x264-params",
            "log=-1:chromaloc=1:slices=1:fullrange=on",
            "-forced-idr",
            "1",
            "-g",
            "1",
        ...
        cmdConv2PNG = [a.ffmpeg, "-y", "-i", h264Path, compPngPath]

        cv.imwrite(pngPath, img)
        subprocess.check_call(cmdConv2H264, stdin=subprocess.DEVNULL)
        subprocess.check_call(cmdConv2PNG, stdin=subprocess.DEVNULL)
        ...
        if os.path.isfile(pngPath):
            os.unlink(pngPath)
        if os.path.isfile(h264Path):
            os.unlink(h264Path)
        if os.path.isfile(compPngPath):
            os.unlink(compPngPath)
...
def main(argv):
    ...
    argp.add_argument(
        "-t", "--tmpDir", default=".", type=str, help="Path to temporary directory."
    )
    argp.add_argument(
        "--slice",
        default=None,
    ...
    argp.add_argument(
        "--ffmpeg", default="/usr/bin/ffmpeg", type=str, help="FFmpeg binary to invoke."
    )
    argp.add_argument(
        "-v",
        "--verbose",
    ...
    with open(pathNameH264, "xb") as fH264, open(
        pathNameLengths, "xb"
    ) as fLengths, open(pathNameTargets, "xb") as fTargets, open(
        pathNameFilenames, "x"
    ) as fFilenames:
